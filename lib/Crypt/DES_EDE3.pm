# $Id: DES_EDE3.pm,v 1.2 2001/04/20 19:41:27 btrott Exp $
...
Creates a new I<Crypt::DES_EDE3> object (really, a collection of three
DES ciphers), and initializes each cipher with part of I<$key>, which
should be at least 24 bytes. If it's longer than 24 bytes, the extra
...
Encrypts an 8-byte block of data I<$block> using the three DES ciphers
in an encrypt-decrypt-encrypt operation.

Returns the encrypted block.

=head2 $ede3->decrypt($block)

Decrypts an 8-byte block of data I<$block> using the three DES ciphers
in a decrypt-encrypt-decrypt operation.
